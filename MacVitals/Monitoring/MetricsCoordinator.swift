...
final class MetricsCoordinator: ObservableObject {
    ...
    func stop() { samplingTask?.cancel(); samplingTask = nil; isRunning = false }

    func handleSleep() {
        stop(); cpuProvider.resetBaseline(); evaluator.reset()
    ...
    func handleWake() {
        cpuProvider.resetBaseline(); evaluator.reset(); start()
    }

    private var currentInterval: TimeInterval { UserDefaults.standard.double(forKey: "samplingInterval").nonZero ?? 2 }
    ...
    private func sample() async {
        ...
        let powerSample = PowerSample(timestamp: Date(),
        ...
        let power = MetricValue(value: assessment, unit: .watts,
        ...
        snapshot = SystemSnapshot(timestamp: Date(), cpu: cpu, memory: memory,
    ...
    private func publishHistory() { cpuHistory = cpuBuffer.values; memoryHistory = memoryBuffer.values }
}
