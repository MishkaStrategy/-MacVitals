    ...
    TabView {
      ...
      VStack(alignment: .leading) {
        Picker("Preset", selection: $settings.selectedPreset) {
          ForEach(MenuPreset.allCases) { Text($0.rawValue.capitalized).tag($0) }
        }
        List {
          ForEach(settings.enabledMetrics) { metric in
            Label(metric.rawValue.capitalized, systemImage: metric.defaultSymbol)
          }
          .onMove(perform: settings.move)
        }
        Text(
          "Drag metrics to change their order. Separate status items remain experimental and are not enabled in v1.0.0."
        )
