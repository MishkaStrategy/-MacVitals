...
nonisolated enum CPUUsageCalculator {
    static func calculate(previous: CPUTicks, current: CPUTicks) throws -> CPUStats {
        guard current.user >= previous.user,
              ...
              current.nice >= previous.nice else { throw CPUCalculationError.counterReset }
        ...
        guard total > 0 else { throw CPUCalculationError.zeroDelta }
        func percent(_ value: UInt64) -> Double { min(100, max(0, Double(value) / Double(total) * 100)) }
        let userPercent = percent(user + nice)
        let systemPercent = percent(system)
        let idlePercent = percent(idle)
        return CPUStats(total: min(100, max(0, userPercent + systemPercent)),
                        user: userPercent, system: systemPercent, idle: idlePercent,
...
final class CPUProvider: @unchecked Sendable {
    ...
    func sample() -> MetricValue<CPUStats> {
        ...
        guard let ticks = readTicks() else {
            return .unavailable(unit: .percent, availability: .providerError,
                                source: .machHostStatistics, message: "host_statistics failed")
        }
        return lock.withLock {
            defer { previous = ticks }
            guard let previous else {
                return .unavailable(unit: .percent, availability: .temporarilyUnavailable,
                                    source: .machHostStatistics, message: "Collecting baseline")
            }
            do {
                let stats = try CPUUsageCalculator.calculate(previous: previous, current: ticks)
                return MetricValue(value: stats, unit: .percent, availability: .available,
                                   quality: .direct, source: .machHostStatistics,
                                   timestamp: now, isEstimated: false, message: nil)
            } catch {
                return .unavailable(unit: .percent, availability: .temporarilyUnavailable,
                                    source: .machHostStatistics, message: "CPU counters reset")
            }
        }
    }
    ...
    private func readTicks() -> CPUTicks? {
        ...
        return CPUTicks(user: UInt64(load.cpu_ticks.0), system: UInt64(load.cpu_ticks.1),
