...
nonisolated enum MenuMetric: String, Codable, CaseIterable, Identifiable, Sendable {
    ...
    var defaultSymbol: String {
        switch self { case .cpu: return "cpu"; case .gpu: return "rectangle.3.group"; case .memory: return "memorychip";
        case .battery: return "battery.75percent"; case .adapterPower: return "bolt.fill"; case .powerStatus: return "gauge.with.dots.needle.67percent" }
    }
}
...
final class SettingsStore: ObservableObject {
    ...
    @Published var samplingInterval: Double { didSet { UserDefaults.standard.set(samplingInterval, forKey: "samplingInterval") } }
    @Published var showInDock: Bool { didSet { NSApp.setActivationPolicy(showInDock ? .regular : .accessory); UserDefaults.standard.set(showInDock, forKey: "showInDock") } }
    @Published var reducedMotion: Bool { didSet { UserDefaults.standard.set(reducedMotion, forKey: "reducedMotion") } }
    ...
    init() {
        ...
        } else { enabledMetrics = MenuPreset.performance.metrics }
    ...
    func reset() {
        selectedPreset = .performance; samplingInterval = 2; reducedMotion = false
    }

    private func persist() {
        if let data = try? JSONEncoder().encode(enabledMetrics) { UserDefaults.standard.set(data, forKey: "enabledMetrics") }
    }
}
