...
fail() {
  printf 'post-single-clock-long-stability: %s\n' "$*" >&2
  exit 1
}
...
print("Post-single-clock long-stability harness self-test passed")
PY
}
...
python3 "${REPORTER}" \
  "${summary_path}" \
  --scenario post-single-clock-six-hour-stability \
...
if resource.get("scenario") != "post-single-clock-six-hour-stability":
...
expected_samples = int(expected_duration / sample_interval) + 1
minimum_samples = max(2, int((expected_duration / sample_interval) * 0.97))
if int(measurement.get("sampleCount", 0)) < minimum_samples:
    raise SystemExit(
        f"long-stability sample count is insufficient: "
        f"actual={measurement.get('sampleCount')} expected≈{expected_samples} minimum={minimum_samples}"
    )
