...
@MainActor
final class CoalescedClockPrimaryHistoryValidationTests: XCTestCase {
  func testLivePrimaryAndHistoryPreserveCadenceWithBoundedClockTolerance() async throws {
    let environment = ProcessInfo.processInfo.environment
    guard
      let readyPath = environment["MACVITALS_COALESCED_CLOCK_HISTORY_READY_FILE"], !readyPath.isEmpty,
      let completePath = environment["MACVITALS_COALESCED_CLOCK_HISTORY_COMPLETE_FILE"], !completePath.isEmpty
    else {
      throw XCTSkip("Coalesced-clock primary/history evidence paths are required")
    }
    ...
    XCTAssertGreaterThanOrEqual(
      revisionAdvance,
      50,
      "Bounded clock tolerance must preserve at least 50 historical revisions over the 70-second shared-cadence window")
    XCTAssertGreaterThanOrEqual(
      liveAdvance,
      55,
      "The live primary consumer must remain fresh while the shared clock uses bounded tolerance")
    ...
    try Data("coalesced-clock-history-and-live-primary-advanced\n".utf8).write(
  ...
  private func waitUntil(
    ...
    XCTFail("Timed out waiting for coalesced-clock primary/history runtime state")
